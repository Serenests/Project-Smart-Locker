...
export default function TransactionDetailTestPage() {
  ...
  const handleFetchReportData = async () => {
    try {
      ...
      let queryParams = `start_date=${reportForm.start_date}&end_date=${reportForm.end_date}`

      // เพิ่ม location_id หรือ group_location_id ตาม role
      if (currentUser?.role === 3) {
        // Role 3 ใช้ location_id ของตัวเอง
        queryParams += `&location_id=${currentUser.locationId}`
      } else if (reportForm.location_id) {
        queryParams += `&location_id=${reportForm.location_id}`
      } else if (reportForm.group_location_id) {
        queryParams += `&group_location_id=${reportForm.group_location_id}`
      }

      console.log('📊 Fetching report data:', queryParams)
  ...
  const handleDownloadCSV = () => {
    ...
    // Column headers
    csvContent += 'รหัสยา,ชื่อยา,จำนวนเติมยา,จำนวนเบิกยา,จำนวนคงเหลือ\n'

    // Data rows
    reportData.items.forEach(item => {
      csvContent += `${item.product_id},${item.product_name},${item.total_restock},${item.total_withdraw},${item.current_stock}\n`
    })

    // Summary section
    csvContent += '\n'
    csvContent += 'สรุปรวม\n'
    csvContent += `จำนวนรายการยาทั้งหมด,${reportData.summary.total_products} รายการ\n`
    csvContent += `จำนวนเติมยารวม,${reportData.summary.total_restock_all} ชิ้น\n`
    csvContent += `จำนวนเบิกยารวม,${reportData.summary.total_withdraw_all} ชิ้น\n`
    csvContent += `จำนวนคงเหลือรวม,${reportData.summary.total_current_stock} ชิ้น\n`

    // สร้าง Blob ด้วย BOM สำหรับ UTF-8 (รองรับภาษาไทย)
    const BOM = '\uFEFF'
    const blob = new Blob([BOM + csvContent], { type: 'text/csv;charset=utf-8;' })

    // สร้าง filename
    const startDateFormatted = reportForm.start_date.replace(/-/g, '')
    const endDateFormatted = reportForm.end_date.replace(/-/g, '')
    const filename = `รายงานยา_${reportData.location_name}_${startDateFormatted}-${endDateFormatted}.csv`
...
const handleDownloadPDF = () => {
    ...
    try {
      // ตรวจสอบว่ามี font base64 หรือไม่
      if (typeof thSarabunBase64 !== 'undefined' && thSarabunBase64.length > 100) {
        doc.addFileToVFS("Kanit-Regular.ttf", thSarabunBase64)  // ✅ เปลี่ยนชื่อ
        doc.addFont("Kanit-Regular.ttf", "Kanit-Regular", "normal")  // ✅ เปลี่ยนชื่อ
        doc.setFont("Kanit-Regular")  // ✅ เปลี่ยนชื่อ
        console.log('✅ Thai font loaded successfully')
      } else {
        // Fallback เป็น Helvetica ถ้าไม่มี Thai font
    ...
    // Period
    doc.setFontSize(11)
    doc.text(`ช่วงเวลา: ${formatDateThai(reportData.start_date)} - ${formatDateThai(reportData.end_date)}`, pageWidth / 2, 38, { align: 'center' })
    ...
    doc.text(`${reportData.summary.total_products} รายการ`, pageWidth / 2 + 45, yPos + 18)
    ...
    doc.setFontSize(12)
    doc.setFont('Kanit-Regular', 'normal')
    doc.text('รายละเอียดยา', margin, yPos)
    yPos += 5

    // Prepare table data
    const tableData = reportData.items.map((item, index) => [
      (index + 1).toString(),
      item.product_id,
      item.product_name,
    ...
    //   '',
    //   'รวมทั้งหมด',
    //   reportData.summary.total_restock_all.toString(),
    //   reportData.summary.total_withdraw_all.toString(),
    //   reportData.summary.total_current_stock.toString()
    // ])
    ...
    autoTable(doc, {
      ...
      // Style for the totals row (last row)
    ...
    doc.setFontSize(12)
    doc.setFont('Kanit-Regular', 'normal')
    doc.text('สรุปภาพรวม', margin, summaryY)
    summaryY += 7

    // Card 1: Total Products (Blue)
    doc.setFillColor(59, 130, 246)
    doc.roundedRect(margin, summaryY, cardWidth, cardHeight, 2, 2, 'F')
    doc.setTextColor(255, 255, 255)
    doc.setFontSize(9)
    doc.setFont('Kanit-Regular', 'normal')
    doc.text('จำนวนรายการยา', margin + cardWidth/2, summaryY + 8, { align: 'center' })
    doc.setFontSize(16)
    doc.setFont('Kanit-Regular', 'normal')
    doc.text(reportData.summary.total_products.toString(), margin + cardWidth/2, summaryY + 18, { align: 'center' })

    // Card 2: Total Restock (Green)
    const card2X = margin + cardWidth + 5
    doc.setFillColor(34, 197, 94)
    doc.roundedRect(card2X, summaryY, cardWidth, cardHeight, 2, 2, 'F')
    doc.setTextColor(255, 255, 255)
    doc.setFontSize(9)
    doc.setFont('Kanit-Regular', 'normal')
    doc.text('จำนวนเติมยารวม', card2X + cardWidth/2, summaryY + 8, { align: 'center' })
    doc.setFontSize(16)
    doc.setFont('Kanit-Regular', 'normal')
    doc.text(reportData.summary.total_restock_all.toString(), card2X + cardWidth/2, summaryY + 18, { align: 'center' })

    // Card 3: Total Withdraw (Red)
    const card3X = margin + (cardWidth + 5) * 2
    doc.setFillColor(239, 68, 68)
    doc.roundedRect(card3X, summaryY, cardWidth, cardHeight, 2, 2, 'F')
    doc.setTextColor(255, 255, 255)
    doc.setFontSize(9)
    doc.setFont('Kanit-Regular', 'normal')
    doc.text('จำนวนเบิกยารวม', card3X + cardWidth/2, summaryY + 8, { align: 'center' })
    doc.setFontSize(16)
    doc.setFont('Kanit-Regular', 'normal')
    doc.text(reportData.summary.total_withdraw_all.toString(), card3X + cardWidth/2, summaryY + 18, { align: 'center' })

    // Card 4: Current Stock (Purple)
    const card4X = margin + (cardWidth + 5) * 3
    doc.setFillColor(139, 92, 246)
    doc.roundedRect(card4X, summaryY, cardWidth, cardHeight, 2, 2, 'F')
    doc.setTextColor(255, 255, 255)
    doc.setFontSize(9)
    doc.setFont('Kanit-Regular', 'normal')
    doc.text('จำนวนคงเหลือรวม', card4X + cardWidth/2, summaryY + 8, { align: 'center' })
    doc.setFontSize(16)
    doc.setFont('bold')
    doc.text(reportData.summary.total_current_stock.toString(), card4X + cardWidth/2, summaryY + 18, { align: 'center' })
    ...
    const startDateFormatted = reportForm.start_date.replace(/-/g, '')
    const endDateFormatted = reportForm.end_date.replace(/-/g, '')
    const filename = `รายงานยา_${reportData.location_name.replace(/\s+/g, '_')}_${startDateFormatted}-${endDateFormatted}.pdf`
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredTransactions.length === 0 ? (
                      ...
                      filteredTransactions.filter((transaction) => transaction.status === 'สำเร็จ').map((transaction) => (
                ...
                <CardContent className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <Badge
                      ...
                      className={selectedTransaction[0].Transaction.status === 'สำเร็จ' ? 'bg-green-500 hover:bg-green-600' : ''}
                    >
      ...
      <Dialog open={isReportDialogOpen} onOpenChange={setIsReportDialogOpen}>
        <DialogContent className="sm:max-w-[500px]">
          <DialogHeader>
            ...
            <DialogDescription>
              เลือกช่วงเวลาและสถานที่เพื่อออกใบรีพอร์ตสรุปการเบิก-เติมยา
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            {/* แสดงตาม Role */}
            {currentUser?.role === 1 && (
              <>
                {/* Role 1: เลือก Group Location ก่อน */}
                <div>
                ...
                <div>
                  <Label htmlFor="location_id">สถานที่ (เฉพาะเจาะจง)</Label>
                  <Select
                    value={reportForm.location_id || "all"}
                    onValueChange={(value) => setReportForm({...reportForm, location_id: value === "all" ? "" : value})}
            ...
            {currentUser?.role === 2 && (
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setReportForm({...reportForm, location_id: value})}
                >
                  <SelectTrigger>
          ...
          {reportData && (
            <div className="space-y-4">
              ...
              <Card className="bg-gray-50">
                <CardContent className="pt-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-sm text-gray-500">สถานที่</p>
                      <p className="font-semibold">{reportData.location_name}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">กลุ่มสถานที่</p>
                      <p className="font-semibold">{reportData.group_location_name}</p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">ช่วงเวลา</p>
                      <p className="font-semibold">
                        {new Date(reportData.start_date).toLocaleDateString('th-TH')} - {new Date(reportData.end_date).toLocaleDateString('th-TH')}
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500">ผู้ออกรีพอร์ต</p>
                      <p className="font-semibold">
                        {currentUser?.firstName} {currentUser?.lastName}
                      </p>
                    </div>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-lg">
                    รายการยา ({reportData.items.length} รายการ)
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {reportData.items.length === 0 ? (
                    <p className="text-center text-gray-500 py-4">ไม่พบข้อมูลในช่วงเวลาที่เลือก</p>
                  ) : (
                    <Table>
                      <TableHeader>
                        <TableRow>
                      ...
                      <TableBody>
                        {reportData.items.map((item) => (
                          <TableRow key={item.product_id}>
                            ...
                            <TableCell className="text-right">
                              <Badge className="bg-green-500">{item.total_restock}</Badge>
                            </TableCell>
                            <TableCell className="text-right">
                              <Badge variant="destructive">{item.total_withdraw}</Badge>
                            </TableCell>
                            <TableCell className="text-right">
              ...
              <Card className="bg-blue-50 border-blue-200">
                <CardHeader>
                  <CardTitle className="text-lg text-blue-700">สรุปรวม</CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-4 gap-4">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-2xl font-bold text-green-600">
                        {reportData.summary.total_restock_all}
                      </p>
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-2xl font-bold text-red-600">
                        {reportData.summary.total_withdraw_all}
                      </p>
                      <p className="text-xs text-gray-500">ชิ้น</p>
                    </div>
                    <div className="text-center">
                      ...
                      <p className="text-2xl font-bold text-blue-700">
                        {reportData.summary.total_current_stock}
                      </p>
                      <p className="text-xs text-gray-500">ชิ้น</p>
                    </div>
                  </div>
