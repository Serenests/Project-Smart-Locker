        ...
        createProvision: async (req, res) => {
            ...
            try {
                const provision = await prisma.locker_Provision.create({
                    data: {
                        locker_id,
                        provision_code: provision_code || await generateUniqueProvisionCode(),
                        expires_at
                    }
                });
                    ...
                    if (search){
                        ...
                        whereCondition = {
                            OR: [
                                ...
                                { Locker: { Location: { location_name: { contains: search, mode: 'insensitive' } } } },
                                { Locker: { locker_location_detail: { contains: search, mode: 'insensitive' } } },
        ...
        getLockerProvisionByCode: async (req, res) => {
            ...
            try {
                ...
                if (provision.is_activated) {
                    return res.status(400).json({ message: 'รหัสการจัดสรรล็อกเกอร์นี้ถูกใช้งานแล้ว' });
                }
